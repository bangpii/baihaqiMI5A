...
const Gallery = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        alert('Gagal memuat data Gallery: ' + error.message)
      } finally {
        setLoading(false)
      }
    }
  ...
  useEffect(() => {
    ...
    console.log('👂 Setting up Socket.io listeners for Gallery...')
    ...
    const handleConnect = () => {
      ...
      // Join room untuk Gallery updates
  ...
  const handleEditData = (gallery) => {
    console.log('✏️ Opening modal for edit:', gallery)
    setIsEdit(true)
    setFormData({
      title: gallery.title,
      tanggal: gallery.tanggal,
      gambar: gallery.gambar || '',
      kategori: gallery.kategori || 'GALLERY'
    })
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isEdit) {
        ...
        url = `http://localhost:5000/api/gallery/${editingId}`
        method = 'PUT'
  ...
  const getImageUrl = (imagePath) => {
    if (!imagePath) return '/gallery/default-gallery.jpg'
                    ...
                    <td className="px-4 py-3 text-sm text-gray-600 font-poppins">
                      <div className="w-16 h-16 bg-gray-200 rounded-lg flex items-center justify-center overflow-hidden">
                        <img 
                          ...
                          onError={(e) => {
                            e.target.src = '/gallery/default-gallery.jpg'
                          }}
                        />
                      </div>
                    </td>
