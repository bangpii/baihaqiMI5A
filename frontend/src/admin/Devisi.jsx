...
const Devisi = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true)
        console.log('🔄 Fetching Devisi data from API...')
        const response = await fetch('http://localhost:5000/api/devisi')
        if (!response.ok) {
          throw new Error(`HTTP error! status: ${response.status}`)
        }
        const data = await response.json()
        console.log('✅ Devisi data received:', data)
        setDataTable(data)
      } catch (error) {
        console.error('❌ Error fetching Devisi data:', error)
        alert('Gagal memuat data Devisi: ' + error.message)
      } finally {
        setLoading(false)
      }
    }
    ...
    console.log('🔌 Initializing Socket.io for Devisi...')
  ...
  useEffect(() => {
    ...
    console.log('👂 Setting up Socket.io listeners for Devisi...')

    const handleDevisiUpdate = (data) => {
      console.log('🔄 Realtime Devisi update received:', data)
      setDataTable(data)
    }

    const handleConnect = () => {
      console.log('🔌 Socket.io connected for Devisi')
      socket.emit('join_devisi_room')
    }

    // Event listeners
    socket.on('connect', handleConnect)
    socket.on('devisi_updated', handleDevisiUpdate)
    ...
    return () => {
      ...
      socket.off('devisi_updated', handleDevisiUpdate)
    }
  }, [socket])
  ...
  const handleTambahData = () => {
    console.log('➕ Opening modal for new Devisi data')
    setIsEdit(false)
    setFormData({
      name: '',
      nomorInduk: '',
  ...
  const handleEditData = (member) => {
    console.log('✏️ Opening modal for edit Devisi:', member)
    setIsEdit(true)
    setFormData({
      name: member.name,
  ...
  const handleCloseModal = () => {
    console.log('❌ Closing Devisi modal')
    setShowModal(false)
    setFormData({
      name: '',
      nomorInduk: '',
  ...
  const handleImageChange = (e) => {
    ...
    if (file) {
      console.log('🖼️ Image selected for Devisi:', file.name)
  ...
  const handleRemoveImage = () => {
    console.log('🗑️ Removing Devisi image')
    setSelectedImage(null)
    setImagePreview(null)
  ...
  const handleSubmit = async (e) => {
    ...
    console.log('🚀 Submitting Devisi form:', { isEdit, editingId, formData, selectedImage })
    ...
    try {
      ...
      Object.keys(formData).forEach(key => {
        if (formData[key]) {
          ...
          console.log(`📦 Devisi form data - ${key}:`, formData[key])
        }
      })
      ...
      if (selectedImage) {
        ...
        console.log('📸 Devisi image appended:', selectedImage.name)
      }

      let url, method
      if (isEdit) {
        // Update existing data
        url = `http://localhost:5000/api/devisi/${editingId}`
        method = 'PUT'
        console.log('🔄 Updating Devisi data at:', url)
      } else {
        // Create new data
        url = 'http://localhost:5000/api/devisi'
        method = 'POST'
        console.log('🆕 Creating Devisi data at:', url)
      }
      ...
      console.log('📡 Devisi response status:', response.status)

      if (!response.ok) {
        const errorText = await response.text()
        console.error('❌ Devisi server error response:', errorText)
        throw new Error(errorText || `HTTP error! status: ${response.status}`)
      }

      const result = await response.json()
      console.log('✅ Devisi success response:', result)
      
      // Socket.io akan otomatis update data via realtime
      handleCloseModal()
      alert(`✅ Data Devisi berhasil ${isEdit ? 'diupdate' : 'ditambahkan'}`)

    } catch (error) {
      console.error(`❌ Error ${isEdit ? 'updating' : 'creating'} Devisi data:`, error)
      alert(`❌ Gagal ${isEdit ? 'mengupdate' : 'menambahkan'} data Devisi: ${error.message}`)
    }
  }

  const handleDeleteData = async (id) => {
    if (!window.confirm('Apakah Anda yakin ingin menghapus data Devisi ini?')) {
      return
    }

    console.log('🗑️ Deleting Devisi data with id:', id)

    try {
      const response = await fetch(`http://localhost:5000/api/devisi/${id}`, {
        method: 'DELETE'
      })

      console.log('📡 Devisi delete response status:', response.status)

      if (!response.ok) {
        const errorText = await response.text()
        console.error('❌ Devisi delete error response:', errorText)
        throw new Error(errorText || `HTTP error! status: ${response.status}`)
      }

      const result = await response.json()
      console.log('✅ Devisi delete success:', result)
      
      // Socket.io akan otomatis update data via realtime
      alert('✅ Data Devisi berhasil dihapus')

    } catch (error) {
      console.error('❌ Error deleting Devisi data:', error)
      alert(`❌ Gagal menghapus data Devisi: ${error.message}`)
    }
  }
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="space-y-4">
        ...
        <div className="flex items-center space-x-2 text-sm text-gray-500 font-poppins">
          ...
          <span className="text-blue-600 font-semibold">Devisi</span>
        </div>
        
        {/* Header dengan Tombol Tambah Data - Desktop */}
        <div className="hidden lg:flex justify-between items-center">
          <div>
            <h2 className="text-3xl font-bold text-gray-800 font-poppins">Data Devisi</h2>
            <p className="text-gray-600 font-poppins mt-1">Kelola data Devisi organisasi</p>
          </div>
          <button 
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="flex justify-between items-center p-6 border-b border-gray-200">
              <h3 className="text-xl font-bold text-gray-800 font-poppins">
                {isEdit ? 'Edit Data Devisi' : 'Tambah Data Devisi'}
              </h3>
              <button 
