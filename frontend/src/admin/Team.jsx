...
const Team = () => {
  ...
  useEffect(() => {
    ...
    console.log('🔌 Initializing Socket.io for Team...')
  ...
  useEffect(() => {
    ...
    console.log('👂 Setting up Socket.io listeners for Team...')
    ...
    const handleConnect = () => {
      console.log('🔌 Socket.io connected for Team')
  ...
  const handleTambahData = () => {
    console.log('➕ Opening modal for new Team data')
    setIsEdit(false)
    setFormData({
      name: '',
      nomorInduk: '',
  ...
  const handleEditData = (member) => {
    console.log('✏️ Opening modal for edit Team:', member)
    setIsEdit(true)
    setFormData({
      name: member.name,
  ...
  const handleImageChange = (e) => {
    ...
    if (file) {
      console.log('🖼️ Image selected for Team:', file.name)
  ...
  const handleRemoveImage = () => {
    console.log('🗑️ Removing Team image')
    setSelectedImage(null)
    setImagePreview(null)
  ...
  const handleSubmit = async (e) => {
    ...
    console.log('🚀 Submitting Team form:', { isEdit, editingId, formData, selectedImage })
    ...
    try {
      ...
      if (selectedImage) {
        ...
        console.log('📸 Team image appended:', selectedImage.name)
      }
      ...
      if (isEdit) {
        ...
        method = 'PUT'
        console.log('🔄 Updating Team data at:', url)
      } else {
        // Create new data
        url = 'http://localhost:5000/api/team'
        method = 'POST'
        console.log('🆕 Creating Team data at:', url)
      }
      ...
      if (!response.ok) {
        ...
        console.error('❌ Team server error response:', errorText)
        throw new Error(errorText || `HTTP error! status: ${response.status}`)
      }

      const result = await response.json()
      console.log('✅ Team success response:', result)
      
      // Socket.io akan otomatis update data via realtime
      handleCloseModal()
      alert(`✅ Data Team berhasil ${isEdit ? 'diupdate' : 'ditambahkan'}`)

    } catch (error) {
      console.error(`❌ Error ${isEdit ? 'updating' : 'creating'} Team data:`, error)
      alert(`❌ Gagal ${isEdit ? 'mengupdate' : 'menambahkan'} data Team: ${error.message}`)
    }
  }

  const handleDeleteData = async (id) => {
    if (!window.confirm('Apakah Anda yakin ingin menghapus data Team ini?')) {
      return
    }

    console.log('🗑️ Deleting Team data with id:', id)
    ...
    try {
      ...
      console.log('📡 Team delete response status:', response.status)

      if (!response.ok) {
        const errorText = await response.text()
        console.error('❌ Team delete error response:', errorText)
        throw new Error(errorText || `HTTP error! status: ${response.status}`)
      }

      const result = await response.json()
      console.log('✅ Team delete success:', result)
      
      // Socket.io akan otomatis update data via realtime
      alert('✅ Data Team berhasil dihapus')

    } catch (error) {
      console.error('❌ Error deleting Team data:', error)
      alert(`❌ Gagal menghapus data Team: ${error.message}`)
    }
  }
