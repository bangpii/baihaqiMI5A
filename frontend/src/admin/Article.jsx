...
const Article = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        alert('Gagal memuat data Article: ' + error.message)
      } finally {
        setLoading(false)
      }
    }
  ...
  useEffect(() => {
    ...
    console.log('👂 Setting up Socket.io listeners for Article...')
    ...
    const handleConnect = () => {
      ...
      // Join room untuk Article updates
      socket.emit('join_article_room')
    }

    // Event listeners
    socket.on('connect', handleConnect)
    socket.on('article_updated', handleArticleUpdate)
    ...
    return () => {
      ...
      socket.off('article_updated', handleArticleUpdate)
    }
  }, [socket])
  ...
  const handleEditData = (article) => {
    console.log('✏️ Opening modal for edit:', article)
    setIsEdit(true)
    setFormData({
      title: article.title,
      desk: article.desk,
      ...
      kategori: article.kategori || 'ARTICLE'
    })
    setEditingId(article.id)
    setSelectedImage(null)
    setImagePreview(article.gambar ? `http://localhost:5000${article.gambar}` : null)
    setShowModal(true)
  }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isEdit) {
        ...
        url = `http://localhost:5000/api/articles/${editingId}`
        method = 'PUT'
  ...
  const getImageUrl = (imagePath) => {
    if (!imagePath) return '/article/default-article.jpg'
                    ...
                    <td className="px-4 py-3 text-sm text-gray-600 font-poppins">
                      <div className="w-16 h-16 bg-gray-200 rounded-lg flex items-center justify-center overflow-hidden">
                        <img 
                          ...
                          onError={(e) => {
                            e.target.src = '/article/default-article.jpg'
                          }}
                        />
                      </div>
                    </td>
